...
function ledger(overrides: Record<string, unknown> = {}) {
  return {
    ...
    findings: Array.from({ length: 14 }, (_, index) => finding(index + 1)),
    ...overrides,
  }
}

describe('release-readiness ledger', () => {
  it('requires the complete A-01 through A-14 inventory and named evidence', () => {
    ...
    expect(validateReleaseLedger(ledger({ findings: [finding(1, { requiredEvidence: [] })] }))).toEqual(
      expect.arrayContaining(['A-01: requiredEvidence must contain named evidence', 'A-14: finding is missing']),
    )
  })
  ...
  it('keeps release blocking while an owned finding is open', () => {
    ...
    expect(result.blockingFindings).toHaveLength(14)
  })
