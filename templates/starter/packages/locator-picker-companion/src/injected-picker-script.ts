...
export function installLocatorPickerOverlay(): void {
  type PickerState = {
    picking: boolean
    submitting: boolean
    hoveredElement: Element | null
    preview: CompanionPickedLocatorPayload | null
    error: string
    elements: {
      root?: HTMLDivElement
      startButton?: HTMLButtonElement
      useButton?: HTMLButtonElement
      pickAgainButton?: HTMLButtonElement
      cancelButton?: HTMLButtonElement
      helperText?: HTMLParagraphElement
      statusText?: HTMLParagraphElement
      selectorValue?: HTMLPreElement
      metadataValue?: HTMLParagraphElement
      errorValue?: HTMLParagraphElement
    }
  }

  const globalState = window as Window & {
    __appraiseLocatorPickerInstalled?: boolean
    __appraiseLocatorPickerDomListenersBound?: boolean
    __appraiseLocatorPickerState?: PickerState
    __appraiseLocatorPickerConfirm?: (payload: CompanionPickedLocatorPayload) => Promise<void>
    __appraiseLocatorPickerCancel?: () => Promise<void>
  }
  ...
  function escapeForSelectorText(value: string): string {
  ...
  function countRoleNameMatches(role: string, accessibleName: string): number {
    let count = 0
  ...
  function buildXPathSelector(element: Element, text: string): string {
    const tagName = element.tagName.toLowerCase()
  ...
  function buildPrimarySelector(element: Element): {
    selector: string
    strategy: CompanionPickedLocatorPayload['strategy']
  } {
  ...
  function buildPreviewPayload(element: Element): CompanionPickedLocatorPayload {
  ...
  function setPicking(enabled: boolean): void {
  ...
  function ensureRoot(): void {
    ...
    if (state.elements.root?.isConnected) {
      return
    }
  ...
  function render(): void {
    ...
    selectorValue.textContent = state.preview?.selector ?? 'No selector picked yet.'
