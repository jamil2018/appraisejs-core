...
Then(
  ...
  async function (
    ...
    try {
      ...
      const elementActiveStatus = await this.page
        .locator(selector)
        .isEnabled({ timeout: 10000 });
      if (isActive) {
        void expect(elementActiveStatus, `Expected ${elementName} to be active`)
          .to.be.true;
      } else {
        void expect(
          elementActiveStatus,
          `Expected ${elementName} NOT to be active`
        ).to.be.false;
      }
    } catch (error) {
      throw new Error(
        `Failed to validate the active status of the element ${elementName}: ${error}`
      );
    }
  }
);
