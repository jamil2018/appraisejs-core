...
## AppraiseJS repository routing

When `src/graphify-out/graph.json` and the matching package scripts exist, use these commands and treat that file as
`GRAPH_PATH` throughout this reference:

```bash
npm run graphify:query -- "QUESTION"
npm run graphify:path -- "SOURCE" "TARGET"
npm run graphify:explain -- "NODE"
```

Do not require or rebuild `graphify-out/graph.json` in AppraiseJS. The wrappers pass the canonical source graph to the
CLI. All generic `graphify-out/graph.json` examples below apply only to repositories without this override; when an
inline fallback is necessary in AppraiseJS, substitute `src/graphify-out/graph.json` exactly.
