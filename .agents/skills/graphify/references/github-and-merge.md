...
These extraction commands need no API key for code-only folders. If a folder also contains docs, papers, or images
and neither `GEMINI_API_KEY` nor `GOOGLE_API_KEY` is already configured, run the normal Graphify skill pipeline for
that folder so the host agent performs semantic extraction. Gemini is an optional faster backend when one of those
two keys is already available; do not request a key or select an unsupported provider backend.
