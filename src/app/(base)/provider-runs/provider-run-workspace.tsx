...
import { Ban, Bot, CheckCircle2, CircleAlert, Clock3, ExternalLink, FileText, Play, ShieldCheck } from 'lucide-react'
...
export function ProviderRunWorkspace({ runs, adapters, targetProjects, plans }: ProviderRunWorkspaceProps) {
  ...
  const [providerKey, setProviderKey] = useState(adapters[0]?.key ?? 'mock-planning')
  const [providerProfile, setProviderProfile] = useState('planning-default')
  ...
  const canLaunch = Boolean(targetProjectId && providerKey && launchPrompt.trim()) && !isPending
