...
const GenerateTestCaseFromTemplate = async ({
  params,
}: {
  params: Promise<{ id: string }>;
}) => {
  const { id } = await params;
  const { data: templateTestCase, error } =
    await getTemplateTestCaseByIdAction(id);
  ...
  const convertedData =
    templateTestCaseToTestCaseConverter(templateTestCaseData);
  ...
  return (
    <div>
      ...
      <HeaderSubtitle>
        Create a new test from a template to execute against your application
      </HeaderSubtitle>
