...
describe('pending comparators', () => {
  it('retains family-owned mismatch reasons and aggregates without entity branches', () => {
    const comparisons = [pendingComparison('sync-plans', 2), pendingComparison('sync-tags', 0)]
    expect(comparisons[0].reasons).toEqual(['2 projected sync-plans record(s) differ from persisted state.'])
    expect(comparisons[1].reasons).toEqual([])
    expect(aggregatePendingComparisons(comparisons)).toEqual({
      counts: { 'sync-plans': 2, 'sync-tags': 0 },
      total: 2,
    })
  })
})
