...
export async function deleteFeatureFile(testSuiteId: string): Promise<boolean> {
  try {
    ...
    if (!testSuite) {
      console.warn(
        `Test suite with ID ${testSuiteId} not found for feature file deletion`
      );
      return false;
    }
  ...
  try {
    ...
    for (const testSuite of testSuites) {
      try {
        ...
        console.error(
          `Error generating feature file for test suite ${testSuite.name}:`,
          error
        );
      }
    }
    ...
    console.log(
      `Regeneration complete. Generated ${generatedFiles.length} feature files.`
    );
