...
export function buildModulePath(
  ...
  while (currentModule) {
    ...
    currentModule =
      modules.find((m) => m.id === currentModule?.parentId) || null;
  }
...
export function buildModulePathFromParent(
  ...
  const path: string[] = [];
  let currentModule: (Module & { parent: { name: string } }) | null =
    selectedModule;
  ...
  while (currentModule) {
    ...
    if (currentModule.parent?.name) {
      ...
      const parentModule = modules.find(
        (m) => m.name === currentModule?.parent?.name
      );
 ...
 */
export function buildAllModulePaths(
  modules: (Module & { parent: { name: string } })[]
): Map<string, string> {
