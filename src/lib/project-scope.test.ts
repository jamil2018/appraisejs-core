...
describe('project scope route gate', () => {
  it('recognizes scoped collection and detail routes', () => {
    ...
    expect(isProjectScopedPath('/plans/plan-1')).toBe(true)
  ...
  it('requires selection only when both URL and cookie scope are absent', () => {
    expect(shouldRequireProjectSelection({ pathname: '/plans' })).toBe(true)
    expect(shouldRequireProjectSelection({ pathname: '/plans', urlProjectId: 'project-1' })).toBe(false)
    expect(shouldRequireProjectSelection({ pathname: '/plans', cookieProjectId: 'project-1' })).toBe(false)
  })

  it('adds the selected project while preserving the requested destination', () => {
    expect(withProjectScope('/plans/plan-1?review=validation', 'project 1')).toBe(
      '/plans/plan-1?review=validation&project=project+1',
    )
