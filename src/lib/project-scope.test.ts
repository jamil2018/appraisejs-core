...
describe('project scope route gate', () => {
  it('recognizes scoped collection and detail routes', () => {
    ...
    expect(isProjectScopedPath('/quality-journeys/journey-1')).toBe(true)
  ...
  it('requires selection only when both URL and cookie scope are absent', () => {
    expect(shouldRequireProjectSelection({ pathname: '/quality-journeys' })).toBe(true)
    expect(shouldRequireProjectSelection({ pathname: '/quality-journeys', urlProjectId: 'project-1' })).toBe(false)
    expect(shouldRequireProjectSelection({ pathname: '/quality-journeys', cookieProjectId: 'project-1' })).toBe(false)
  })

  it('adds the selected project while preserving the requested destination', () => {
    expect(withProjectScope('/quality-journeys/journey-1?review=triage', 'project 1')).toBe(
      '/quality-journeys/journey-1?review=triage&project=project+1',
    )
