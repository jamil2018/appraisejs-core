...
const TEMPLATE_SCRIPTS = {
  ...
  'appraise:setup': 'npm run setup',
  'appraise:sync': 'npm run sync-all',
...
async function patchAndRead(dir: string, pm: 'npm' | 'pnpm' | 'yarn' | 'bun') {
  await fs.writeJson(path.join(dir, 'package.json'), { name: 'appraise', scripts: { ...TEMPLATE_SCRIPTS } });
...
describe('patchPackageJsonScripts', () => {
  it('rewrites package.json scripts to use pnpm instead of npm', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'create-appraise-patch-'));
    try {
      ...
      expect(pkg.scripts['appraise:setup']).toBe('pnpm run setup');
      expect(pkg.scripts['appraise:sync']).toBe('pnpm run sync-all');
  ...
  it('rewrites scripts for npm (keeps legacy-peer-deps and npx)', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'create-appraise-patch-npm-'));
    try {
  ...
  it('rewrites scripts for yarn', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'create-appraise-patch-yarn-'));
    try {
      ...
      expect(pkg.scripts['appraise:setup']).toBe('yarn run setup');
  ...
  it('rewrites scripts for bun', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'create-appraise-patch-bun-'));
    try {
      ...
      expect(pkg.scripts['appraise:sync']).toBe('bun run sync-all');
  ...
  it('no-ops when package.json does not exist', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'create-appraise-noop-'));
    try {
