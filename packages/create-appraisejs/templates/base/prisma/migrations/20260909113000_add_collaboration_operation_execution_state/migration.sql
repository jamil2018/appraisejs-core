...
-- These rows predate persisted execution plans. They must not be inferred from
-- mutable repository state or made executable merely because the new step
-- table exists. Terminal and already-blocked history is preserved unchanged;
-- every active legacy operation is durably stopped and must be re-prepared.
UPDATE "CollaborationOperation"
SET
  "state" = 'BLOCKED',
  "blockerJson" = json_object(
    'code', 'LEGACY_PLAN_MISSING',
    'legacyState', "state",
    'remediation', 'REPREPARE_OPERATION'
  )
WHERE "state" NOT IN ('COMPLETED', 'FAILED', 'CANCELLED', 'SUPERSEDED', 'BLOCKED');
