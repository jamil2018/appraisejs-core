...
import { runLocatorStepOperation, runPageStepOperation } from './step-operations'

describe('structured Step Definition operations', () => {
  it('dispatches an allowlisted locator operation with stored-variable references', async () => {
    const fill = vi.fn().mockResolvedValue(undefined)
    await runLocatorStepOperation(
      { fill } as never,
      'fill',
      '[{"$stored":"customerName"}]',
      '{"timeout":5000}',
  ...
  it('rejects unsupported locator operations and options before touching Playwright', async () => {
    ...
    await expect(runLocatorStepOperation(locator as never, 'evaluate', '[]', '{}', vi.fn())).rejects.toThrow(
      'Unsupported locator operation: evaluate',
    )
    await expect(
      runLocatorStepOperation(locator as never, 'click', '[]', '{"unknown":true}', vi.fn()),
  ...
  it('rejects malformed argument shapes and invalid stored-variable references', async () => {
    await expect(runLocatorStepOperation({} as never, 'fill', '{}', '{}', vi.fn())).rejects.toThrow(
      'Arguments JSON must be an array',
    )
    await expect(runLocatorStepOperation({} as never, 'fill', '[{"$stored":""}]', '{}', vi.fn())).rejects.toThrow(
  ...
  it('dispatches safe page operations and rejects arbitrary JavaScript evaluation', async () => {
    ...
    await runPageStepOperation(page as never, 'setViewportSize', '[{"width":1280,"height":720}]', '{}', vi.fn())
    expect(setViewportSize).toHaveBeenCalledWith({ width: 1280, height: 720 })

    await expect(runPageStepOperation(page as never, 'evaluate', '[]', '{}', vi.fn())).rejects.toThrow(
  ...
  it('bounds explicit waits', async () => {
    ...
    await expect(
      runPageStepOperation({ waitForTimeout } as never, 'waitForTimeout', '[120001]', '{}', vi.fn()),
