...
export type ProviderRegistrationUpdateInput = {
  providerKey: string
...
async function ensureAdapterRegistration(adapter: ProviderAdapter, client: PrismaClient) {
  const registration = await client.providerAdapterRegistration.upsert({
    ...
    create: {
      ...
      enabled: adapter.key === 'mock-planning',
      executablePath: adapter.defaultExecutable,
      defaultProfile: adapter.defaultProfile,
      defaultModel: adapter.defaultModel,
      probeStatus: adapter.key === 'mock-planning' ? 'installed' : 'not_probed',
      probeMessage: adapter.setupMessage,
      launchEnabled: adapter.key === 'mock-planning',
      settingsJson: stringifyJson({
        setupMessage: adapter.setupMessage,
        launchableWhenProbed: adapter.launchableWhenProbed,
      }),
    },
    update: {
      displayName: adapter.displayName,
  ...
  const legacyBackfill: Prisma.ProviderAdapterRegistrationUpdateInput = {
    ...(registration.executablePath ? {} : { executablePath: adapter.defaultExecutable }),
    ...(registration.settingsJson
      ? {}
      : {
          settingsJson: stringifyJson({
            setupMessage: adapter.setupMessage,
  ...
  if (adapter.key === 'mock-planning' && !isLaunchableRegistration(registration)) {
    Object.assign(legacyBackfill, {
      enabled: true,
  ...
  if (Object.keys(legacyBackfill).length === 0) return registration
  return client.providerAdapterRegistration.update({ where: { key: adapter.key }, data: legacyBackfill })
}

async function ensureProviderRegistrations(client: PrismaClient) {
  await Promise.all(providerAdapters.map(adapter => ensureAdapterRegistration(adapter, client)))
}

function registrationSettings(registration: { settingsJson: string | null }) {
  return parseJson<JsonRecord>(registration.settingsJson) ?? {}
}

function isLaunchableRegistration(registration: {
  enabled: boolean
  launchEnabled: boolean
  probeStatus: string
  capabilitiesJson: string
}) {
  const capabilities = parseJson<{ launch?: boolean }>(registration.capabilitiesJson)
  return Boolean(
    registration.enabled &&
      registration.launchEnabled &&
      registration.probeStatus === 'installed' &&
      capabilities?.launch,
  )
}
...
export async function listProviderRegistrations(client: PrismaClient = prisma) {
  await ensureProviderRegistrations(client)
  const registrations = await client.providerAdapterRegistration.findMany({ orderBy: { displayName: 'asc' } })
  return registrations.map(registration => ({
    ...registration,
    capabilities: parseJson(registration.capabilitiesJson),
    settings: registrationSettings(registration),
    launchable: isLaunchableRegistration(registration),
  }))
}

export async function listProviderAdapters(client: PrismaClient = prisma) {
  await ensureProviderRegistrations(client)
  const registrations = await client.providerAdapterRegistration.findMany({
    where: { enabled: true, launchEnabled: true, probeStatus: 'installed' },
    orderBy: { displayName: 'asc' },
  })
  return registrations.filter(isLaunchableRegistration)
}

export async function updateProviderRegistration(
  input: ProviderRegistrationUpdateInput,
  client: PrismaClient = prisma,
) {
  const adapter = getProviderAdapter(input.providerKey)
  if (!adapter) throw new ServiceError(`Provider adapter "${input.providerKey}" is not available.`, 'VALIDATION', 400)
  const current = await ensureAdapterRegistration(adapter, client)
  const settings = input.settings === undefined ? registrationSettings(current) : (input.settings ?? {})
  const sanitizedSettings = Object.fromEntries(
    Object.entries(settings).filter(([key]) => !/secret|token|api[_-]?key|password/i.test(key)),
  )
  return client.providerAdapterRegistration.update({
    where: { key: input.providerKey },
    data: {
      ...(input.executablePath !== undefined ? { executablePath: input.executablePath?.trim() || null } : {}),
      ...(input.defaultProfile !== undefined ? { defaultProfile: input.defaultProfile?.trim() || null } : {}),
      ...(input.defaultModel !== undefined ? { defaultModel: input.defaultModel?.trim() || null } : {}),
      ...(input.enabled !== undefined ? { enabled: input.enabled } : {}),
      ...(input.launchEnabled !== undefined
        ? { launchEnabled: input.launchEnabled && current.probeStatus === 'installed' }
        : {}),
      settingsJson: stringifyJson({
        ...sanitizedSettings,
        setupMessage: adapter.setupMessage,
...
export async function probeProviderRegistration(providerKey: string, client: PrismaClient = prisma) {
  ...
  const registration = await ensureAdapterRegistration(adapter, client)
  const result = await adapter.probe({ executablePath: registration.executablePath })
  const launchEnabled = adapter.launchableWhenProbed && result.launchEnabled && result.status === 'installed'
  return client.providerAdapterRegistration.update({
    where: { key: providerKey },
    data: {
      executablePath: result.executablePath ?? registration.executablePath ?? adapter.defaultExecutable,
...
export async function createProviderWorkflowRun(input: ProviderRunCreateInput, client: PrismaClient = prisma) {
  ...
  if (!isLaunchableRegistration(adapterRegistration)) {
    throw new ServiceError(
      `Provider adapter "${providerKey}" is not enabled and launchable. Probe and enable it in Settings first.`,
      'VALIDATION',
      400,
    )
  }
...
async function readTargetPlanState(targetProjectId: string, client: PrismaClient) {
  return client.planProjection.findMany({
    where: { targetProjectId },
    select: { planId: true, revision: true, sourceHash: true, updatedAt: true },
    orderBy: { updatedAt: 'desc' },
  })
}

function planStateChanged(
  before: Awaited<ReturnType<typeof readTargetPlanState>>,
  after: Awaited<ReturnType<typeof readTargetPlanState>>,
) {
  const beforeById = new Map(before.map(plan => [plan.planId, `${plan.revision}:${plan.sourceHash}`]))
  return after.some(plan => beforeById.get(plan.planId) !== `${plan.revision}:${plan.sourceHash}`)
}
...
async function launchProviderWorkflowRun(runId: string, client: PrismaClient = prisma) {
  ...
  const beforePlanState = await readTargetPlanState(run.targetProjectId, client)
  ...
  try {
    const result = await adapter.launch({
      ...
      hubProjectPath: process.cwd(),
      baseUrl: process.env.APPRAISE_BASE_URL ?? process.env.NEXT_PUBLIC_APPRAISE_BASE_URL ?? 'http://127.0.0.1:3000',
      executablePath: run.providerAdapter?.executablePath,
      providerProfile: run.providerProfile ?? run.providerAdapter?.defaultProfile,
      providerModel: run.providerAdapter?.defaultModel,
      settings: parseJson(run.providerAdapter?.settingsJson ?? null),
    })
    const postRunRepoSnapshot = await readGitSnapshot(run.targetProject.canonicalPath)
    const afterPlanState = await readTargetPlanState(run.targetProjectId, client)
    const completedWithoutPlanChange =
      result.status === 'completed' && !planStateChanged(beforePlanState, afterPlanState)
    const status = completedWithoutPlanChange ? 'recovery_required' : result.status
    const completedAt = ['completed', 'failed', 'cancelled'].includes(status) ? new Date() : null
    const recoveryEvent: NormalizedProviderEvent | null = completedWithoutPlanChange
      ? {
          type: 'provider_run_failed',
          payload: {
            message: 'Provider exited successfully but no new or revised target-bound Appraise plan was detected.',
    ...
    await client.$transaction(async transaction => {
      ...
      if (recoveryEvent) await appendProviderEvent(transaction, run.id, recoveryEvent)
      await transaction.providerWorkflowRun.update({
        ...
        data: {
          ...
          failureReason: recoveryEvent?.payload?.message ? String(recoveryEvent.payload.message) : undefined,
        },
      })
    })
  } catch (error) {
