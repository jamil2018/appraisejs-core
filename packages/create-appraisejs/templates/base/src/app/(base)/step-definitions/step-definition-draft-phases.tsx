...
function CompositionFields({
  ...
  const addChild = (contract: ReadyCompositionChildContract) => {
    ...
    if (children.some(existing => compositionContractKey(existing.step) === compositionContractKey(child.step))) {
      setSearchError('This Step Definition is already in the composition.')
      void rejectReadyStepDefinitionSelectionAction({
        step: { id: child.step.id, version: child.step.version },
        reason: 'overlap',
      })
      return
    }
