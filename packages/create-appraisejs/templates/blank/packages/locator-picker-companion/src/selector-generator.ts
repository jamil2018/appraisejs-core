...
import type { CompanionPickedLocatorPayload, CompanionPickedLocatorStrategy } from './types.js'
...
function buildCssSelector(snapshot: ElementSnapshot): string {
...
function buildXPathSelector(snapshot: ElementSnapshot): string {
  if (snapshot.id) {
...
function buildPrimarySelector(snapshot: ElementSnapshot): {
  selector: string
...
async function getElementSnapshot(elementHandle: ElementHandle): Promise<ElementSnapshot> {
  return elementHandle.evaluate(node => {
    const element = node as Element
    const normalizeText = (value: string | null | undefined): string => (value ?? '').replace(/\s+/g, ' ').trim()
    const isLikelyStableIdentifier = (value: string | null | undefined): value is string => {
      if (!value) {
        return false
      }
    ...
    const getLabelText = (candidate: Element): string => {
      if (!['input', 'textarea', 'select'].includes(candidate.tagName.toLowerCase())) {
        return ''
      }

      const input = candidate as HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement
    ...
    const getAccessibleName = (candidate: Element): string => {
    ...
    const getRole = (candidate: Element): string => {
    ...
    return {
      ...
      stableClasses: Array.from(element.classList || [])
        .filter((className): className is string => isLikelyStableIdentifier(className))
        .slice(0, 2),
...
export async function generatePickedLocatorPayload(
  page: Page,
