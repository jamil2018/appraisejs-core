...
describe('CLI E2E', () => {
  ...
  it('patchPackageJsonScripts rewrites real template scripts for chosen package manager', async () => {
    ...
    expect(pkgAfter.scripts['appraisejs:sync']).toBe('pnpm run sync-step-definitions')
    ...
    expect(pkgAfter.scripts['sync-step-definitions']).toContain('pnpm exec ')
  ...
  it('patchPackageJsonScripts rewrites npx-using scripts for bun', async () => {
    ...
    expect(pkgAfter.scripts['sync-step-definitions']).toContain('bunx ')
