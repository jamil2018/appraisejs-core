...
6. Create the plan with `planning_session_create` when available, or with `plan_create` followed by
   `plan_wait_for_review`.
7. Present Appraise and browser links only after durable `plan_review_ready` evidence.
8. After review readiness, call `plan_wait_for_approval` with a bounded wait or poll mode when the host cannot safely
   hold a long-poll open, then enter standby. Pending approval is not completion.
9. If approval is still pending, keep the returned `nextAfterSequence`, review links, lifecycle, and content hash so a
   later turn can resume the same wait.
