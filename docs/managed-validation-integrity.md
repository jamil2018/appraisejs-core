...
Appraise-managed quality work is target-bound from requirements through sealed evidence. A target project owns its requirement graph, Quality Plan revisions, validation designs, publications, runtime capsules, TestRuns, Assessments, and Evidence Receipts. Cross-project reuse is explicit and identity-bound.
...
Changes to this architecture require focused ownership, coverage, contract-parity, validation-publication, Assessment, capsule, TestRun, scaffold, and end-to-end quality workflow tests. Root changes must be synchronized with `npm --prefix packages/create-appraisejs run prepare-template`; generated scaffold or Graphify output must not be hand-edited.
